document.addEventListener('DOMContentLoaded', () => {
  ...
  function groupFilter(group) {
    if (group === 'g1') return QUESTIONS.filter(q => q.round <= 8);
    if (group === 'g2') return QUESTIONS.filter(q => q.round > 8);
    return QUESTIONS;
  }
  ...
  function findQuestionById(id) {
    return QUESTIONS.find(q => questionId(q) === id);
  }
  ...
  function renderHistory() {
    ...
    historyList.innerHTML = hist.slice(0, 10).map(h => {
      ...
      return `<div class="history-row">
        ...
        <span>${GROUP_LABELS[h.group] || h.group}</span>
  ...
  function renderResumeBanner() {
    ...
    resumeInfo.textContent =
      `問題 ${progress.idx + 1}/${progress.order.length}　${MODE_LABELS[progress.mode] || progress.mode}　${GROUP_LABELS[progress.group] || ''}　正解 ${progress.score.correct}/${progress.score.total}`;
  ...
  function showQuestion() {
    ...
    qContext.innerHTML = q.context.replace('___BLANK___', '<span class="blank">＿＿＿＿＿＿</span>');
    updateProgress();
  ...
  function showAnswer() {
    ...
    const isLikelyMatch = userAns.length > 0 && q.answers.some(a => normalize(a) === normalize(userAns));

    correctAnswerDiv.innerHTML = `
      <div class="user-answer">あなたの答え：${userAns ? escapeHtml(userAns) : '（未入力）'}</div>
      <div class="correct-label">正解：<code>${q.display}</code></div>
      ${isLikelyMatch ? '<div class="auto-hint">自動チェック：入力と一致しているようです</div>' : ''}
  ...
  function renderDndQuestion(q) {
    ...
    dndFeedback.className = '';
    dndNextBtn.style.display = 'none';
    dndChoices.innerHTML = '';

    const correct = q.display;
    const choices = shuffle([correct, ...(q.distractors || [])]);
  ...
  document.getElementById('start-btn').addEventListener('click', () => {
    ...
    currentGroup = document.querySelector('input[name="group"]:checked').value;
